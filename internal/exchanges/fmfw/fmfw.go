...
func (c *Client) Name() string { return "fmfw" }

func (c *Client) URI(*client.Context) string { return "wss://api.fmfw.io/api/3/ws/public" }
...
func (c *Client) Subscribe(ctx *client.Context, send func(string)) {
	...
	c.id++
	send(fmt.Sprintf(`{"method":"subscribe", "ch":"ticker/price/1s", "params":{"symbols": [%s]}, "id": %d}`,
		strings.Join(pairs, ","), c.id))
}

type response struct {
	Data map[string]struct {
		C jsonutil.Float `json:"c"` // close / last price
		Q jsonutil.Float `json:"q"` // quote volume
	} `json:"data"`
}
